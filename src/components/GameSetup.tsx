...
const TAGLINE_DEFAULTS = [
  'Eye of the tiger', 'Treble or nothing', 'Drinks on me if I bust',
  'Just here for the banter', 'Born to throw', 'Dartfather',
  'Ton-eighty dreams', 'Slow and steady',
];
...
export default function GameSetup({ onStartGame, onBack, sessionCode }: Props) {
  const [playerCount, setPlayerCount] = useState(2);
  const [mode, setMode] = useState<'501' | '301'>('501');
  const [legsPerSet, setLegsPerSet] = useState(3);
  const [setsToWin, setSetsToWin] = useState(1);
  const [players, setPlayers] = useState<PlayerDraft[]>([
    { name: 'Morgan', avatar: '🔥', tagline: 'The Hammer' },
    { name: 'Jess',   avatar: '🎯', tagline: 'Bullseye' },
    { name: 'Dave',   avatar: '🍺', tagline: 'Just Dave' },
    { name: 'Ruth',   avatar: '🦊', tagline: 'Late bloomer' },
    { name: 'Sam',    avatar: '🐺', tagline: 'Wolfpack' },
  ]);
  ...
  return (
    <div className="page">
      <div className="bar-header">
        ...
        <div className="bar-status"><span>PRESS START WHEN READY</span></div>
      </div>
      {sessionCode && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {players.slice(0, playerCount).map((p, i) => (
              <div key={i} className="player-row" style={{ position: 'relative' }}>
                <div className="avatar-pick" onClick={() => setAvatarPickerFor(avatarPickerFor === i ? null : i)}>
                  {p.avatar}
                </div>
                {avatarPickerFor === i && (
                  <div className="avatar-popover" style={{ left: 72, top: 60 }}>
                    {AVATAR_SET.map(a => (
                      <button key={a} onClick={() => { updatePlayer(i, { avatar: a }); setAvatarPickerFor(null); }}>{a}</button>
                    ))}
                  </div>
                )}
                <div className="tagline-row">
                  <input value={p.name} onChange={e => updatePlayer(i, { name: e.target.value })} placeholder={`Player ${i+1}`} />
                  <input className="tagline" value={p.tagline} onChange={e => updatePlayer(i, { tagline: e.target.value })} placeholder={TAGLINE_DEFAULTS[i] || 'Walk-on quote'} />
                </div>
                <div style={{ color: 'var(--ink-2)', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase' }}>
                  P{i+1}
                </div>
              </div>
            ))}
